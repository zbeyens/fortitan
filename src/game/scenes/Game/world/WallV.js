...
export default class WallV extends EntityV {
  constructor(entity) {
    ...
    // TODO: only once!
    const bmd = this.game.make.bitmapData(
      cfg.building.thickness,
      cfg.building.length
    );
    bmd.rect(
      0,
    ...
    this.sprite = this.scene.add.sprite(0, 0, bmd);
    this.centerAnchor(this.sprite);
    this.scene.buildingGroup.add(this.sprite);
  ...
  update(delta) {
    ...
    const { x } = this.entity.state;
    const { y } = this.entity.state;
